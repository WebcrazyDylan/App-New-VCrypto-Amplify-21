...
const getCoinAmount = async (coinPortfolioCoinId, userId) => {
  ...
  console.log("getCoinAmount -> porftolio coin data");
...
const getUsdAmount = async (usdPortfolioCoinId, userId) => {
  ...
  const coinData = await ddb.getItem(params).promise();
  console.log("getUsdAmount -> usd coin data");
  // console.log(coinData);
...
const getCoin = async (coinId) => {
  ...
  const coinData = await ddb.getItem(params).promise();
  console.log("getCoin -> coin data");
  // console.log(coinData);
  return coinData;
};

const canBuyCoin = (coin, amountToBuy, usdAmount) => {
  console.log("canBuyCoin -> return Boolean");
...
const buyCoin = async (
  ...
  // decrease USD
  const newUsdAmount =
    usdAmount - parseFloat(coin.Item.currentPrice.N) * amountToBuy;
  const params = {
    Item: {
      id: { S: usdPortfolioCoinId },
...
const sellCoin = async (
  ...
  // increase USD
  const newUsdAmount =
    usdAmount + parseFloat(coin.Item.currentPrice.N) * amountToSell;
  const params = {
    Item: {
      id: { S: usdPortfolioCoinId },
      __typename: { S: "PortfolioCoin" },
