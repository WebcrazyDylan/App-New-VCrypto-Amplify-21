...
exports.handler = async (event, context) => {
  ...
  const Items = data.map((entry) => ({
    ...
    valueChange24H: {
      N: entry.price_change_percentage_1h_in_currency.toString()
    },
    valueChange1D: {
      N: entry.price_change_percentage_24h_in_currency.toString()
    },
    valueChange7D: {
      N: entry.price_change_percentage_7d_in_currency.toString()
    },
  ...
  lambda.invoke(invokeParams, function (err, data) {
    console.log("invoking Lambda Function Start");

    if (err) console.log(err, err.stack);
    else {
      console.log("Lambda Triggered End");
    }
  });
